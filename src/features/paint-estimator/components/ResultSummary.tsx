...
export function ResultSummary({ result, hasAnyWall, activeWalls, canType, coats, doorCount, windowCount }: { result: EstimatorResult; hasAnyWall:boolean; activeWalls:{widthFt:number;heightFt:number}[]; canType:CanType; coats:1|2; doorCount:number; windowCount:number }){
  ...
  if(!hasAnyWall){
    return (
      <div className="rounded-2xl border border-line bg-white p-6 shadow-[var(--shadow-card)]">
        <h2 className="text-sm font-semibold tracking-widest uppercase text-ink-muted">Your estimate</h2>
  ...
  if(result.cutoutExceedsGross){
    return (
      <div className="rounded-2xl border border-line bg-white p-6 shadow-[var(--shadow-card)]">
        ...
        <div className="mt-4 rounded-xl bg-paper-2 border border-line p-4 text-sm guard-enter">
          <div className="flex justify-between"><span className="text-ink-muted">Gross wall area</span><span className="font-medium">{fmt(result.grossAreaSqFt)} sq ft</span></div>
          <div className="flex justify-between mt-1"><span className="text-ink-muted">Cutout area</span><span className="font-medium">− {fmt(result.cutoutAreaSqFt)} sq ft</span></div>
          <div className="flex justify-between mt-2 pt-2 border-t border-line font-semibold"><span>Net paintable</span><span>0 sq ft</span></div>
        </div>
        <div aria-live="polite" aria-atomic="true" className="sr-only">{announced}</div>
      </div>
    )
  ...
  return (
    <div className="rounded-2xl border border-line bg-white p-6 shadow-[var(--shadow-card)]">
      <h2 className="text-sm font-semibold tracking-widest uppercase text-ink-muted">Your estimate</h2>
      <div className="mt-4 rounded-xl bg-paper-2 border border-line p-4 text-sm">
        <div className="flex justify-between"><span className="text-ink-muted">Gross wall area</span><span className="font-medium number-fade">{fmt(result.grossAreaSqFt)} sq ft</span></div>
        <div className="flex justify-between mt-1"><span className="text-ink-muted">Minus doors & windows</span><span className="font-medium">− {fmt(result.cutoutAreaSqFt)} sq ft</span></div>
        <div className="flex justify-between mt-1"><span className="text-ink-muted">Net paintable area</span><span className="font-semibold">{fmt(result.netAreaSqFt)} sq ft</span></div>
        <div className="flex justify-between mt-2 pt-2 border-t border-line"><span className="text-ink-muted">× {coats} coat{coats>1?"s":""}</span><span className="font-medium">{fmt(result.totalCoverageNeededSqFt)} sq ft needed</span></div>
        <div className="flex justify-between mt-1"><span className="text-ink-muted">÷ {CAN_COVERAGE[canType]} sq ft per {canType}</span><span className="font-semibold">{result.cansNeeded} {unit}</span></div>
      </div>
      {showSuccess ? (
        ...
        <>
          <p className="mt-5 text-[22px] font-bold leading-tight">Buy <span className="text-accent number-fade">{result.cansNeeded} {unit}</span></p>
          <p className="mt-2 text-sm leading-relaxed text-ink-muted number-fade">{leftoverNote}</p>
        </>
      )}
